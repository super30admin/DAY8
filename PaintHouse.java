public class PaintHouse {
    public static int paintCosts(int[][] costs) {
        ...
        int[][] dp = new int[houses][3]; //columns 3 indicate the three colors R,B,G

        //Assigning the bottom row of dp as the same values of costs
        ...
        for(int i=houses-2; i>=0; i--) { //O(n) T.C
            ...
            dp[i][2] = costs[i][2] + Math.min(dp[i+1][0], dp[i+1][1]); //O(n) S.C
        }
